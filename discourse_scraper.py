...
DOWNLOAD_DIR = os.path.join(os.getcwd(), "downloaded_threads")

# Create the directory if it doesn't exist
if not os.path.exists(DOWNLOAD_DIR):
    print("downloaded_threads dir created!")
...
def login_and_save_auth(playwright):
    ...
    print(
        "🌐 Please log in manually using Google. Then press ▶️ (Resume) in Playwright bar."
    )
    page.pause()
...
def scrape_posts(playwright):
    ...
    with open(os.path.join(DOWNLOAD_DIR, "discourse_posts.json"), "w") as f:
        json.dump(filtered_posts, f, indent=2)

    print(
        f"✅ Scraped {len(filtered_posts)} posts between {DATE_FROM.date()} and {DATE_TO.date()}"
    )
    browser.close()
