        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as file:
                ...
                for post in posts:
                    ...
                    # Clean content (optional, depending on whether you're handling HTML)
...
def parse_inline_frontmatter(raw: str):
    """
    Parses inline frontmatter string like:
    'title: "..."original_url: "..."downloaded_at: "..."'
    Returns a dict with keys: title, original_url, downloaded_at
    """
    fields = {}
    keys = ["title", "original_url", "downloaded_at"]
    for key in keys:
        start = raw.find(f'{key}:"') if f'{key}:"' in raw else raw.find(f'{key}: "')
        if start == -1:
            continue
        start += (
            len(f'{key}:"') if raw[start + len(key) + 1] == '"' else len(f'{key}: "')
        )
        end = raw.find('"', start)
    ...
    for file_name in tqdm(markdown_files, desc="Processing Markdown files"):
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as file:
                ...
                # Extract and remove inline frontmatter
                title = ""
                original_url = ""
                downloaded_at = ""
                ...
                if content.startswith("---"):
                    ...
                    if end_index != -1:
                        ...
                        meta = parse_inline_frontmatter(frontmatter_raw)
                        title = meta.get("title", "")
                        original_url = meta.get("original_url", "")
                        downloaded_at = meta.get("downloaded_at", "")
                        content = content[
                            end_index + 3 :
                        ].strip()  # Remove frontmatter block
